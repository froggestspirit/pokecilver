...
//  Bit 7: print leading zeros if set
	...
	BIT_B(5);  // bit 5, b
	IF_Z goto _main;  // jr z, .main
	BIT_B(7);  // bit 7, b
	IF_NZ goto _moneyflag;  // jr nz, .moneyflag
	BIT_B(6);  // bit 6, b
	IF_Z goto _main;  // jr z, .main
...
_main:
	...
	AND_A(0xf);  // and $f
	CP_A(1);  // cp 1
	IF_Z goto _byte;  // jr z, .byte
	CP_A(2);  // cp 2
	IF_Z goto _word;  // jr z, .word
//  maximum 3 bytes
	LD_A_de;  // ld a, [de]
	LDH_addr_A(hPrintNumBuffer + 1);  // ldh [hPrintNumBuffer + 1], a
	INC_DE;  // inc de
...
_start:
	...
	CP_A(3);  // cp 3
	IF_Z goto _three;  // jr z, .three
	CP_A(4);  // cp 4
	IF_Z goto _four;  // jr z, .four
	CP_A(5);  // cp 5
	IF_Z goto _five;  // jr z, .five
	CP_A(6);  // cp 6
	IF_Z goto _six;  // jr z, .six
...
_two:
	SET_PC(0x32EEU);
	DEC_E;  // dec e
	IF_NZ goto _two_skip;  // jr nz, .two_skip
	LD_A(0xf6);  // ld a, "0"
	LDH_addr_A(hPrintNumBuffer + 0);  // ldh [hPrintNumBuffer + 0], a
...
_mod_10:
	SET_PC(0x32F9U);
	CP_A(10);  // cp 10
	IF_C goto _modded_10;  // jr c, .modded_10
	SUB_A(10);  // sub 10
	INC_C;  // inc c
	goto _mod_10;  // jr .mod_10
...
_modded_10:
	...
	OR_A_C;  // or c
	IF_NZ goto _money;  // jr nz, .money
	CALL(mPrintNum_PrintLeadingZero);  // call .PrintLeadingZero
	goto _money_leading_zero;  // jr .money_leading_zero
...
_money:
	...
	INC_E;  // inc e
	DEC_E;  // dec e
	IF_NZ goto _money_leading_zero;  // jr nz, .money_leading_zero
	INC_HL;  // inc hl
	LD_hl(0xe8);  // ld [hl], "."
...
_PrintYen:
	...
	LDH_A_addr(hPrintNumBuffer + 0);  // ldh a, [hPrintNumBuffer + 0]
	AND_A_A;  // and a
	IF_NZ goto _stop;  // jr nz, .stop
	BIT_D(5);  // bit 5, d
	IF_Z goto _stop;  // jr z, .stop
	LD_A(0xf0);  // ld a, "¥"
	LD_hli_A;  // ld [hli], a
	RES_D(5);  // res 5, d
...
_loop:
	...
	CP_A_B;  // cp b
	IF_NC goto _skip2;  // jr nc, .skip2
	LDH_A_addr(hPrintNumBuffer + 1);  // ldh a, [hPrintNumBuffer + 1]
	OR_A(0);  // or 0
	IF_Z goto _skip3;  // jr z, .skip3
	DEC_A;  // dec a
	LDH_addr_A(hPrintNumBuffer + 1);  // ldh [hPrintNumBuffer + 1], a
	LDH_A_addr(hPrintNumBuffer + 2);  // ldh a, [hPrintNumBuffer + 2]
...
_skip2:
	...
	CP_A_B;  // cp b
	IF_NC goto _skip4;  // jr nc, .skip4
	LDH_A_addr(hPrintNumBuffer + 2);  // ldh a, [hPrintNumBuffer + 2]
	AND_A_A;  // and a
	IF_NZ goto _skip5;  // jr nz, .skip5
	LDH_A_addr(hPrintNumBuffer + 1);  // ldh a, [hPrintNumBuffer + 1]
	AND_A_A;  // and a
	IF_Z goto _skip6;  // jr z, .skip6
	DEC_A;  // dec a
	LDH_addr_A(hPrintNumBuffer + 1);  // ldh [hPrintNumBuffer + 1], a
	XOR_A_A;  // xor a
...
_skip1:
	...
	LDH_A_addr(hPrintNumBuffer + 0);  // ldh a, [hPrintNumBuffer + 0]
	OR_A_C;  // or c
	IF_Z goto _PrintLeadingZero;  // jr z, .PrintLeadingZero
	LDH_A_addr(hPrintNumBuffer + 0);  // ldh a, [hPrintNumBuffer + 0]
	AND_A_A;  // and a
